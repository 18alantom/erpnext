...
frappe.ui.form.on("Quality Review", {
	department(frm) {
			...
			callback: function (data) {
				...
				for (var i in objectives) {
					...
					frm.fields_dict.reviews.get_value()[i].uom =
						objectives[i].uom;
					frm.fields_dict.reviews.get_value()[i].ownership =
						objectives[i].ownership;
					frm.fields_dict.reviews.get_value()[i].owner_name =
						objectives[i].owner_name;
				}
		...
		frappe.call({
			method:
				"erpnext.quality_management.doctype.quality_review.quality_review.get_interval_start_and_end_date",
			args: { department },
			callback: function (r) {
